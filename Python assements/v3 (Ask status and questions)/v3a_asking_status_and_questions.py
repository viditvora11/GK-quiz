#asking status
status = input("\nAre you ready for testing your general knowledge through our quiz? \n press y to continue or x to exit : ")#Asking for input
if status == "y" or status == "yes" : #Using if function
    print("\nThank you for choosing our quiz for testing your general knowledge lets keep going") #Printing according to the user input
elif status == 'x' or status == 'no': #Using elif function
    exit() #Exiting the quiz if user says no
...
ans=input("What is the most famous song in the world in 2020?\n\na.shape of you\nb.Despasito\nc.closer\n\nYour answer : ") #Asking for input
if ans == 'shape of you' or ans == 'Shape of you' or ans =='A' or ans == 'a': #Checking answers
    print("Correct") #Telling if their answer is right
    score+=1 #Adding score
    print("Your score is",score) #Showing them the score
else: #Using else function
    print("Oops incorrect the correct answer is :" ,Q1) #Showing them the right answer
    if score <=0:#Keeping the score same as their answer is wrong
        score = 0
        print("Your score is",score) #Showing them the score
