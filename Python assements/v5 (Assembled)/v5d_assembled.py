...
def greet():#using def to call the function many times easily.
    global name #to print the name out of def
...
q_number=[i for i in range(len(gkquiz))]#using while len for the user if they want to play the quiz again.
...
while True:#Using while true loop
    while r>0:#using for range which is better than while len
        ...
        while True:#Using while true loop
            ...
            if user_answer == 'a' or user_answer == 'b' or user_answer == 'c' or user_answer == 'd':
                ...
                del q_number[0] #using del to not reapeat the questions
                r-=1
                break
            else:
    ...
    print("The precentage of the score is",(round(score/total*100,2)),"%")#showing percentage of the score
    ...
    if score == total:
        print('!'*31)
        print("You have won 100 flybuys points.")
        print('!'*31)
    print('='*73)
    if input('Do you want to play again enter y to continue or any other key to exit: ') in ['Y','Yes','y','yes','YES','YEs','yES','YeS']:#asking if the users to play again.
        print('='*73)
        rounds()
    else:
        exit()
