...
import datetime
import time
#assigning varibles for later use
a = datetime.datetime.now()
b = 20
c = 100

#using list and dictnary to contain questions, options and the right answer.
gkquiz =[
[
...
def greet():#using def to call the function many times easily.
    while True:#starting a loop
        name = input("Please enter your name here : ")#asking name
        if name.replace(' ','').isalpha():#using replace to allow spaces 
...
def age():
    while True:
        ...
        if age.replace(' ','').isnumeric():#using replace to allow spaces after the answer
            if 7< int(age)<100: #allowing age till 7 to 100 only
                break
            else:
...
def rule():
    rule = input("\nDo you want to read the rules or continue without the rules? \nPress y to learn the rules or any other key to continue without knowing the rules : ").lower()#using .lower to allow capitals without writing in capitals in the next line when giving options
    rule = rule.replace(' ','')
...
while len(gkquiz)>0:
    ...
    while True:
        ...
        if user_answer == 'a' or user_answer == 'b' or user_answer == 'c' or user_answer == 'd':#giving second chances
            if user_answer == answer:#checking answers
                print("===============================")
                print("nailed it keep it up, good work")
                print("===============================")
                score +=1 #Showing score and increasing score
                print("================")
            ...
            del gkquiz[0]#using del to not reapeat the questions
            break
        else:
...
print("The precentage of the score is",score/b*c,"%")#showing percentage of the score
...
time.sleep(7)#pause the code for 7 seconds giving the user time to read.
